...
Interseptor is a security-testing tool, so we take vulnerabilities **in Interseptor itself**
seriously.
...
> (<https://github.com/Veyal/interseptor/security/advisories/new>)
...
**In scope** — bugs in Interseptor that put *its own users* at risk, for example:
...
**Out of scope** — using Interseptor to test *other* systems (that is its purpose), and any
weaknesses it *reports about a target application* you point it at.

## Supported versions

Security fixes land on the **latest release** (and `main`). Interseptor is a single-maintainer
...
`interseptor update` downloads a prebuilt binary from the matching GitHub release and, when the
...
build from source instead of using a prebuilt binary or `interseptor update`.

## Windows CA-directory permissions

The local CA's private key directory (`~/.interseptor/ca/`) is created with restrictive POSIX-style
...
`~/.interseptor/ca/` themselves (e.g. via `icacls`).
