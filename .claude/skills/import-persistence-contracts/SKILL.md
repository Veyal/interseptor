...
- Publish generated IDs back onto caller-owned structs only after the transaction commits. Assigning
  an ID immediately after `LastInsertId` leaks a nonexistent durable identity when a later metadata
