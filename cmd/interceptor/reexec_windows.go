...
// reexecProject starts a fresh Interceptor on the named project and then exits
...
// quit so it can take over the proxy/control listeners. INTERCEPTOR_REEXEC tells
...
func reexecProject(exe, target string) error {
	...
	cmd.Env = append(os.Environ(), "INTERCEPTOR_REEXEC=1")
