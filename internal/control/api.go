...
var apiRoutes = []apiRoute{
	...
	{"POST", "/api/findings", "Create a curated finding. Body: {title, severity?, status?, target?, impact?, why?, cwe?, environment?, fix?, cvss?, verificationInstructions?, body?, flowIds?, tags?, detail?, evidence?} — title required (stub OK); tags are report-scope labels (cms/api/…)"},
	{"GET", "/api/findings/{id}", "Get one finding with body blocks, PoC flows, tags, and ready/missing completeness"},
	{"PATCH", "/api/findings/{id}", "Update a finding (only fields sent are changed). Body: any subset of {severity, status, title, target, impact, why, cwe, environment, fix, cvss, verificationInstructions, body, detail, evidence, tags}"},
	...
	{"POST", "/api/findings/{id}/flow-preview", "Render a flow as an Interseptor-styled PNG and attach it as finding image evidence. Body: {flowId, side?, pretty?, layout?, theme?, caption?, position?} — defaults: pretty=true, layout=horizontal (request left / response right), theme=light"},
	...
	{"PUT", "/api/settings", "Update settings (rebinds proxy/control listeners). Body: any subset of {proxyAddr, proxyAddrs, controlAddr, upstreamProxy, upstreamProxyCA, oobEnabled, captureScopeOnly, suppressBrowserTelemetry, suppressAndroidTelemetry, invisibleProxy, originTLSVerify, tlsBypassHosts, originTLSVerifyBypassHosts, autoBypassOnPinFailure} — only fields present are changed"},
...
var mcpDescriptor = map[string]any{
	...
	"tools": []map[string]string{
		...
		{"name": "create_finding", "desc": "Record a structured vulnerability finding with description + impact (durable memory); optional tags for report scope"},
		{"name": "list_findings", "desc": "List findings (with PoC flows), filter by severity/status/tag"},
		...
		{"name": "render_flow_preview", "desc": "Render a flow as an Interseptor-styled PNG preview; optionally attach to a finding"},
