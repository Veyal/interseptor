...
var apiRoutes = []apiRoute{
	...
	{"POST", "/api/findings", "Create a curated finding. Body: {title, severity?, status?, target?, detail?, evidence?, impact?, cvss?, body?, flowIds?} — title required; body is a JSON-blocks array string [{type:'text',md}|{type:'flow',flowId,note}] for the interleaved text/flow narrative; flowIds optionally attaches PoC flows on create"},
	{"GET", "/api/findings/{id}", "Get one finding"},
	{"PATCH", "/api/findings/{id}", "Update a finding (only fields sent are changed). Body: any subset of {severity, status, title, target, detail, evidence, impact, cvss, body}"},
