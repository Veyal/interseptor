...
func URLAuthority(scheme, host string, port int) string {
	return strings.ReplaceAll(URLHost(scheme, host, port), "%", "%25")
}

// URLHost formats the corresponding raw net/url.URL.Host field. Unlike
// URLAuthority it leaves an IPv6 zone's percent sign unescaped because
// url.URL.String performs that escaping.
func URLHost(scheme, host string, port int) string {
